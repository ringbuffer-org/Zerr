...
// class mc_zerr_features_tilde : public object<mc_zerr_features_tilde>, public vector_operator<> {
//  private:
//     int outlet_count { 1 };
//     std::unique_ptr<ZerrFeatures> zerr_feature;
//     zerr::t_featureNames ft_names; // Keep track of allocated feature names

//  public:
//     // ==========================
//     // Basic informations
//     // ==========================
//     MIN_DESCRIPTION { "[zerr.features~] xxxx"
//                       "[zerr.features~] xxxx"
//                       "xxxxxxxxxxx" };
//     MIN_TAGS { "DSP" };
...
//         // Create outlets
//         outlet_count = args.size();
//         for (auto i = 0; i < outlet_count; ++i) {
//             auto an_outlet = std::make_unique<outlet<>>(this, "(signal) output signals for each loudspeaker", "signal");
//             m_outlets.push_back(std::move(an_outlet));
//         }

//         // zerr data structure for saving feature names
//         ft_names.names = (char**)malloc(args.size() * sizeof(char*));
//         ft_names.num = args.size();

//         // copy arguments to ft_names structure
//         cout << "ft_names: ";
//         for (int i = 0; i < args.size(); i++) {
//             ft_names.names[i] = strdup(std::string(args[i]).c_str());
//             cout << ft_names.names[i] << " | ";
//         }
//         cout << endl;

//         // system config to initialize zerr: sample rate, block size
//         zerr::SystemConfigs sys_cnfg;
//         sys_cnfg.sample_rate = (float)c74::max::sys_getsr();
//         sys_cnfg.block_size = (int)c74::max::sys_getblksize();
//         cout << " sample_rate: " << sys_cnfg.sample_rate;
//         cout << " | block_size: " << sys_cnfg.block_size << endl;

//         // Allocate memory if needed
//         // create & initialize ZerrFeatures instance
//         // create & initialize ZerrFeatures instance
//         zerr_feature = std::make_unique<ZerrFeatures>(sys_cnfg, ft_names);
//         if (!zerr_feature)
//             error("zerr_feature construction failed");
//         if (zerr_feature->initialize())
//             error("zerr_feature initialization failed");
//     }
...
//     ~mc_zerr_features_tilde()
//     {
//         // Free memory for feature names if they exist
//         if (ft_names.names) {
