...
void* zerr_features_tilde_new(t_symbol* s, int argc, t_atom* argv)
{
    ...
    // parse feature names from arguments
    zerr::FeatureNames ft_names;
    for (int i = 0; i < argc; i++) {
        if (argv[i].a_type == A_SYMBOL) {
            ft_names.push_back(atom_getsymbol(argv + i)->s_name);
        }
        else {
            return NULL;
        }
...
{
    zerr_features_tilde_class =
        class_new(gensym("zerr_features~"), (t_newmethod)zerr_features_tilde_new,
                  (t_method)zerr_features_tilde_free, (size_t)sizeof(zerr_features_tilde),
                  CLASS_DEFAULT, A_GIMME, 0);

    class_addmethod(zerr_features_tilde_class, (t_method)zerr_features_tilde_dsp, gensym("dsp"),
                    A_CANT, A_NULL);
