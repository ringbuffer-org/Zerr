 ...
 * @brief AudioFeatures Class Puredata Wrapper - Provides interface between Pure Data and the core
...
/**
 * @class ZerrFeatures
 * @brief Main wrapper class that interfaces between Pure Data and the core audio feature extraction
 * functionality
 *
 * This class handles the initialization, audio processing, and cleanup of audio feature extraction
 * operations. It manages the data flow between Pure Data's audio system and the internal feature
 * processing chain.
 */
class ZerrFeatures {
  public:
     ...
     */
    ZerrFeatures(zerr::SystemConfigs sys_cnfg, zerr::FeatureNames ft_names);
    /**
     ...
     * @param n_vec The actual size of audio vectors to process (may be smaller than system block
     * size)
     */
  ...
  private:
    ...
    zerr::FeatureBank*
        bank; /**< Core component that implements the feature extraction algorithms */
};
