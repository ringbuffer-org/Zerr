...
FrequencyTransformer::FrequencyTransformer(int L){
    ...
    for(int i=0; i<frame_size; i++){fft_in[i]=0;}
...
void FrequencyTransformer::fft(){
    fftw_execute(p_fft);
}

void FrequencyTransformer::ifft(){
    fftw_execute(p_ifft);
}

void FrequencyTransformer::power_spectrum(){
   for(int i=0; i<fft_size; i++){
    power_spec[i] = (1.0 / (2.0*(float) fft_size)) * (fft_out[i][0] *fft_out[i][0]  + fft_out[i][1]*fft_out[i][1]);
   }
}
