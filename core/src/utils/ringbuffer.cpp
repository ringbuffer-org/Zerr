...
RingBuffer::RingBuffer(size_t capacity)
    : buffer(capacity, 0.0f), head(0), tail(0), size(0) {}
...
void RingBuffer::enqueue(const Block& block) {
    assert(block.size() <= buffer.size() &&
           "Block size must be smaller than buffer size.");
...
void RingBuffer::get_samples(Sample* output_buffer, size_t buf_len) {
    ...
    assert(buf_len == buffer.size() &&
           "Output buffer size should be equal to ringbuffer size!");
