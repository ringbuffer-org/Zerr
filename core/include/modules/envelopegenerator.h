...
namespace zerr
{
    ...
    public:
        const int numInlet =
            3;         ///< Number of inlets: main(1), spread(2), volume(3).
         ...
         * trajectory
         */
        EnvelopeGenerator(SystemConfigs systemCfgs, ConfigPath speakerCfgs,
                          Mode genMode);
        /**
    ...
    private:
        ...
        SystemConfigs
            systemCfgs; /**< System configurations: sample rate, block size etc. */

        ConfigPath
            speakerCfgs;  /**< Path to the speaker array setup configuration file */
        ...
        Logger
            *logger;                  /**< Logger object for printing logs to all kinds of console */
        OnsetDetector *onsetDetector; /**< Detector for identifying onset triggers in the input signal */

        std::map<Index, size_t>
            indexChannelLookup; /**< index to channel reverse lookup table */
        /**
