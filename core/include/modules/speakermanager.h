 ...
 * 
 * This header file defines the Speaker and SpeakerManager classes used to manage
 * speaker configurations and behaviors in the zerr namespace. It includes functionalities
 * for initializing speaker instances with specific configurations, managing speaker
 * arrays, and performing various operations related to speaker selection and properties.
 */
 ...
 * of a speaker, including its index, position (in both Cartesian and spherical coordinates),
 * and orientation (yaw and pitch).
 */
class Speaker {
 public:
    /**
     ...
     * @param position Position of the speaker in Cartesian and spherical coordinates.
    ...
    /**
     * @brief Print all configurations of the speaker, including index, position, and orientation.
     */
    void printAll();
 ...
 private:
    ...
    Position position;          ///< Position of the speaker in both Cartesian and spherical coordinates.
    ...
    void _print_position();     ///< Internal method to print the speaker's position.
    void _print_orientation();  ///< Internal method to print the speaker's orientation.
};


/**
 * @class SpeakerManager
 * @brief Manages speaker array configurations and output speaker selection behaviors.
 * 
 * This class is responsible for loading speaker array configurations from a file,
 * initializing speakers based on the configurations, and managing the selection
 * of output speakers based on various criteria.
 */
class SpeakerManager {
 public:
    /**
     * @brief Construct a new SpeakerManager object and set the configuration file path.
     ...
     * @param trajVal The trajectory input value used to determine speaker selection.
     * @return Pair A pair of speaker indexes selected based on the trajectory value.
     */
     ...
     * This method is intended for calculating how audio should be panned between two
     * speakers based on a specific trajectory value.
    ...
    /**
     * @brief Get the indexes of speakers that are geometrically closest to the given position.
     * This method is marked as obsolete and is planned for removal in future versions.
     * @param pos The target position in either Cartesian or spherical coordinates.
     * @param mask A boolean mask to include or exclude certain speakers.
     * @param coordinate The coordinate system of the pos parameter ("Cartesian" or "spherical").
    ...
    /**
     * @brief Select a speaker index based on a trigger signal and the current mode.
     * @param trigger The trigger signal value.
     * @param mode The operation mode that determines how the trigger affects speaker selection.
    ...
    /**
     * @brief Get a vector of distances from a specific speaker index to all other speakers.
     * @param spkrIdx The index of the speaker from which distances are measured.
    ...
    /**
     * @brief Activate or deactivate speakers based on the given action and speaker indexes.
    ...
    /**
     * @brief Set the trajectory vector, which defines a sequence of speaker indexes for audio playback.
    ...
    /**
     * @brief Set or update the topology matrix based on specified actions and speaker indexes.
     * The topology matrix is used to manage spatial relationships between speakers.
     * @param action The action to be performed ("set", "add", "del").
     * @param spkrIdxes The speaker indexes to which the action will be applied.
     */
    ...
    /**
     * @brief Print parameters related to the speaker manager's configuration and state.
     */
    ...
    /**
     * @brief Print the topology matrix, showing spatial relationships between speakers.
     */
    void printTopoMatrix();


    /**
     * @brief Print the trajectory vector, showing the sequence of speaker indexes for audio playback.
     */
    void printTrajectoryVector();


 private:
    Logger *logger;               ///< Pointer to a Logger object for logging messages in different environment.
    ConfigPath speakerArrayPath;  ///< Path to the speaker array configuration file.
    YAML::Node speakerArrayNode;  ///< YAML Node structure containing the loaded speaker array configuration.

    std::map<Index, Speaker> speakers;       ///< Map of speaker indexes to Speaker objects.
    std::map<Index, Params> distanceMatrix;  ///< Map of each speaker index and the distance to other speakers

    Index                       currIdx;      ///< Current active speaker index.
    Indexes                     actvSpkIdx;   ///< Vector of indexes for currently active speakers.
    Indexes                     trajVector;   ///< Vector of speaker indexes defining the speaker trajectory.
    TopoMatrix                  topoMatrix;   ///< Matrix defining the topology or spatial relationship between speakers.
    ...
    /**
     * @brief Finds the nearest speaker index from a given index within a list of candidate indexes.
