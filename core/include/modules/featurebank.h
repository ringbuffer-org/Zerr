...
namespace zerr
{
    ...
    private:
        ...
        std::vector<fe_ptr>
            activated_features; /**< Vector of pointers to activated feature objects */

        RingBuffer
            ring_buffer; /**< Ring buffer to hold previous audio samples for analysis */

        FrequencyTransformer freq_transformer; /**< FFT wrapper to perform frequency analysis
                                                  on audio signal input */

        AudioInputs
            x; /**< Structure holding different types of feature inputs */

        FeaturesVals
            y; /**< Map containing extracted feature names and their values */
